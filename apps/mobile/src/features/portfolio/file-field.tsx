 ...
 * transcripts). Picks a JPG/PNG/PDF via the system document picker, uploads
 * it into the private `certificates` bucket and writes the resulting storage
 * path into the bound form field. External https links stay supported as a
 * no-upload alternative (and remain how pre-upload rows were stored).
 */
...
interface FileFieldProps {
  label: string;
  /** '' | storage path (`<uid>/<file>.<ext>`) | https URL. */
  value: string;
...
export function FileField({ label, value, onChange, hint }: FileFieldProps) {
  ...
  const pick = useCallback(async () => {
    ...
    try {
      ...
      if (!mimeType && (asset.mimeType ?? '').startsWith('image/')) {
      ...
      const path = await upload.mutateAsync({ localUri: uri, mimeType });
      onChange(path);
